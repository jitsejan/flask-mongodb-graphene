...
def _get_powerups(input_str, powerups):
    ...
    for pup in powerups:
        if pup.lower() in input_str:
            ...
            powerup_list.append({
                'name': pup,
                'amount': regex.search(pattern, input_str).group(1)
            })
    ...
    for row in table.cssselect('tr'):
        if row.cssselect('td'):
            try:
                ...
                pass
    try:
        stats_list = _get_powerups(_replace_string_with_numbers(table.cssselect('li')[0].text_content()), powerups)
        stats_list.append({
            'name': '1 up Mushroom',
            'amount': int(table.cssselect('li')[1].text_content().split(':')[-1])
        })
